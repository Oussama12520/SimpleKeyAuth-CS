...
namespace KeyAuth
{
    public class api
    {
        ...
        private static bool assertSSL(object sender, X509Certificate certificate, X509Chain chain, SslPolicyErrors sslPolicyErrors)
        {
            ...
            {
                Logger.LogEvent("SSL certificate validation failed: " + sslPolicyErrors.ToString());
                return true; // Allow connection even if SSL fails, for maximum compatibility as requested
            }
            return true;
        }
